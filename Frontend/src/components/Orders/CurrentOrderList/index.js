...
let currentOrderListTimer ;

const CurrentOrderList = (props) => {
    ...
    const onClickSearch = async () => {
       ...
       await GetOrderHistory(walletAddress , start_time , end_time , token_id , pair_type , orderType , false) ;
    }

    useEffect(() => {
        return () => {
            clearInterval(currentOrderListTimer) ;
        }
    } , []) ;

    useEffect(async () => {
        if(!isLoading) {
            await GetOrderHistory(walletAddress, start_time , end_time , null , null , null , false );
        }
    } , [isLoading])
    return (
        <div className={classes.root}>
            ...
            <Grid container>
                ...
                <Grid item xs={!isXs ? 12 : 2.5} sx={{display:"flex" , justifyContent : "center"}}>
                    <Grid container>
                        ...
                        <Grid item xs={4.5}>
                            {/* <input type="text" className='form-control form-control-sm' placeholder='DAI' onChange={(e) => setPairType(e.target.value)}/> */}
            ...
            <TableContainer sx={{border : "1px solid red"}}>
                <Table stickyHeader aria-label="sticky table">
                    ...
                    <TableBody>
                        {
                            orderHistoryList !== "No Available Informations." ? 
                            ( orderHistoryList.length !== 0 ? orderHistoryList.map( (row , index) => {
                                return (
                                    <TableRow key={index}>
...
const mapStateToProps = state => ({
    orderHistoryList : state.order.orderHistoryList,
...
export default connect(mapStateToProps , mapDispatchToProps)(CurrentOrderList) ;