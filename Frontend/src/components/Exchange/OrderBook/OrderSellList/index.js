...
const useStyles = makeStyles((theme) => ({
    ...
    progress : {
        ...
        top : 0,
        height : "100%" ,
        width : props => props.max === 0 ? '100%' : `${ props.total * 100 / props.max }%`,
        zIndex : "-1000" ,
    },
    price : {
                            ...
                            orderSellList.length !== 0 ? orderSellList.map((item , index) => {
                                return (
                                    <TableRow key={index} onClick={ () => emitEventToMarketing(item.price) }>
                                        <ProgressBar total={Number(orderSellList[0].price - item.price)} max={Number(orderSellList[0].price - orderSellList[orderSellList.length-1].price)} />
                                        <TableCell className={classes.price}>{Number(item.price).toFixed(decimal)}</TableCell>
                                        <TableCell>{Number(item.remain_amount).toFixed(6)}</TableCell>
                                        <TableCell>{Number(Number(item.price) * Number(item.remain_amount)).toFixed(6)}</TableCell>
                                    </TableRow>
                                )
                            }) : <>
                            </>
