...
export const UploadDocumentation = (formData) => async dispatch => {
    try {
        let res = await axios.post(`${config.BACKEND_API_URL}stake/upload` , formData) ;
        console.log(res);
        if(res.status === 200){
            console.log('get success'+res);
            dispatch({
                type : ActionTypes.UploadDocumentation ,
                payload : "SUCCESS"
            })
        } else {
            console.log("error1"+res);
            dispatch({
                type : ActionTypes.UploadDocumentationError ,
                payload : "ERROR"
            })
        }
    } catch (err) {
