...
// import web3 from '../../../connection/web3.js';
...
const WalletManagement = (props) => {
    ...
    const { tokenBalanceInfo , GetTokenBalanceInfo, walletAddress  } = props ;
    ...
    const setTimerForTokenBalanceList =  async (searchStr) => {
        ...
        tokenBalanceInfoTimer = setInterval(async () => {
            ...
            await GetTokenBalanceInfo(walletAddress , searchStr) ; 
            return ;
        } , 15000 )
    }
    ...
    const searchForMoreTokens = async (token_symbol) => {
        ...
        await GetTokenBalanceInfo(walletAddress , token_symbol) ; 
    ...
    const onDeposit = (tokenAmount , symbol ,logo_url , token_id , token_address , decimal) => {
    ...
    const onWithdraw = (tokenAmount , symbol ,logo_url , token_id , token_address, decimal) => {
        setAmount(tokenAmount) ;
        setLogoUrl(logo_url) ;
        setSymbol(symbol) ;
        setTokenId(token_id) ;
    ...
    const GetEthBalanceInfo = async () => {
        if(walletAddress) {
            let ans = await provider.getBalance(walletAddress) ;
            await setEthAmount(parseInt(ans) / 1000000000000000000) ;
        }
    ...
    useEffect(async () => {
        if(tokenBalanceInfo.length !== 0 && walletAddress) {   
                                            ...
                                            if( !isZeroChecked || ( isZeroChecked && element.balanceOfwallet * element.rate !== 0)) {
                                                return (
                                                    <TableRow key={index}>
                                                        ...
                                                        <TableCell>
                                                            {
                                                                element.symbol === "ETH" ? Number((Number(ethAmount) + Number(element.balanceOfCaladex) + Number(element.orderOfCaladex)) * element.rate).toFixed(4) 
                                                                : Number((Number(element.balanceOfwallet) + Number(element.balanceOfCaladex) + Number(element.orderOfCaladex)) * element.rate).toFixed(4)
                                                            }
                                                        </TableCell>
                                                        <TableCell>
                                                            <ButtonGroup variant="contained" aria-label="outlined primary button group">
                                                                <Button className={classes.btnDeposit} onClick={(e) => onDeposit( element.symbol !== "ETH" ? Number( element.balanceOfwallet ) : Number(ethAmount) , element.symbol , element.logo_url , element._id , element.address, element.decimal)} disabled={deposit_tokens_array[element._id] === true ? false : true}>
                                                                    {  deposit_tokens_array[element._id] === true ? "Deposit" : <><CircularProgress size={20} style={{color : "white"}}/>...Deposit</> }
                                                                </Button>
                                                                <Button className={classes.btnWithDraw} onClick={() => onWithdraw(Number( element.balanceOfCaladex ), element.symbol , element.logo_url , element._id , element.address, element.decimal) } disabled={withdraw_tokens_array[element._id] === true ? false : true}>
